...
(defmethod-f name ((g saveable-graph))
  (with-slots (name) g
    (if (and (slot-boundp g 'name) name)
        (if (functionp name)
            (funcall name)
            name)
        "unnamed")))
  ...
  (let* ((elsd (element sd))
    ...
    (oget-bind (gw gh) element ("clientWidth" "clientHeight")
      (let* ((graph (graph elsd))
        ...
        (labels ((get-mime (ext)
                   (let* ((mime '(("png" . "image/png")
                                  ("svg" . "image/svg+xml;charset=utf-8")
                                  ("jpg" . "image/jpeg")
                                  ("jpeg" . "image/jpeg")))
                          (fnd1 (assoc ext mime :test #'equal))
                          (fnd2 (if fnd1 fnd1 (assoc "png" mime :test #'equal))))
                     (values (cdr fnd2)
                             (car fnd2)
                             fnd1))))
          (destructuring-bind (full-name mime-type) (let* ((nam (name elsd))
                                                           (pos (position #\. nam :from-end t))
                                                           (ext (if pos (subseq nam (1+ pos)) "png")))
                                                      (multiple-value-bind (mime ext fnd) (get-mime ext)
                                                        (list (format nil "~A.~A" (if fnd (subseq nam 0 pos) nam) ext)
                                                              mime)))
            (labels ((get-svg ()
